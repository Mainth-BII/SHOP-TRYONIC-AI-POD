"""Build v28 Excel — Updated from v27 with 6 new features."""
...
MD = os.path.join(BASE, "test_cases_suite_v28.md")
today = datetime.date.today().strftime("%Y-%m-%d")
OUTPUT = os.path.join(BASE, f"TC_POD-TShirt-Platform_ExecutionSummary_v28_{today}_v2.xlsx")
VER = 28
...
def detect_screen(feature, module, tc_id, title):
    ...
    # HOME (merged: ERROR PAGES)
    if feature == "HOME":
        if tid.startswith('TC_ERR') or tid.startswith('TC_TOAST'):
            if 'toast' in mod or 'notification' in mod:
                return "Toast/Snackbar"
            if '404' in mod or 'network' in mod:
                return "Error Page"
            if 'direct url' in mod or 'navigation' in mod:
                return "SPA Routing"
            return "Error Page"
        return "MH Trang chủ"
    ...
    if feature == "LOGIN":
        ...
        if 'responsive' in mod:
            if 'login' in ttl or 'đăng nhập' in ttl:
                return "MH Đăng nhập"
            if 'signup' in ttl or 'đăng ký' in ttl:
                return "MH Đăng ký"
            return "MH Đăng nhập"
        return "MH Đăng nhập"

    # DESIGN STUDIO (merged: MY DESIGNS)
    if feature == "DESIGN STUDIO":
        if tid.startswith('TC_MYDES'):
            return "MH My Designs"
    ...
    # ĐẶT HÀNG (merged: GIỎ HÀNG)
    if feature == "ĐẶT HÀNG":
        if tid.startswith('TC_CART'):
            return "MH Giỏ hàng"
    ...
    # THANH TOÁN (merged: XÁC NHẬN ĐƠN HÀNG)
    if feature == "THANH TOÁN":
        if tid.startswith('TC_CONF'):
            return "MH Xác nhận đơn"
    ...
    # ─── NEW v28 FEATURES ───
    if feature == "GIỎ HÀNG (CART)":
        if 'responsive' in mod or 'zoom' in mod:
            return "MH Giỏ hàng"
        if any(k in mod for k in ['empty', 'trống']):
            return "MH Giỏ hàng - Empty"
        if any(k in mod for k in ['qty', 'số lượng']):
            return "MH Giỏ hàng"
        if any(k in mod for k in ['delete', 'xóa']):
            return "MH Giỏ hàng"
        if any(k in mod for k in ['checkout', 'thanh toán']):
            return "MH Giỏ hàng → Checkout"
        if any(k in mod for k in ['persist', 'multiple']):
            return "MH Giỏ hàng"
        return "MH Giỏ hàng"

    if feature == "PROFILE / ACCOUNT":
        if 'responsive' in mod or 'zoom' in mod:
            return "MH Profile"
        if any(k in mod for k in ['avatar']):
            return "MH Profile - Avatar"
        if any(k in mod for k in ['edit', 'phone', 'logout']):
            return "MH Profile"
        return "MH Profile"

    if feature == "MY DESIGNS (Thiết kế của tôi)":
        if any(k in mod for k in ['empty']):
            return "MH My Designs - Empty"
        if any(k in mod for k in ['load', 'delete', 'rename']):
            return "MH My Designs"
        return "MH My Designs"

    if feature == "AI TRY-ON (Thử đồ với AI)":
        if any(k in mod for k in ['upload']):
            return "DS - AI Try-on Upload"
        if any(k in mod for k in ['generate', 'download']):
            return "DS - AI Try-on"
        if any(k in mod for k in ['canvas']):
            return "DS - AI Try-on"
        return "DS - AI Try-on"

    if feature == "XÁC NHẬN ĐƠN HÀNG (ORDER CONFIRMATION)":
        return "MH Xác nhận đơn"

    if feature == "ERROR PAGES & NOTIFICATIONS":
        if 'toast' in mod or 'notification' in mod:
            return "Toast/Snackbar"
        if '404' in mod or 'error' in mod or 'network' in mod:
            return "Error Page"
        if 'direct url' in mod or 'navigation' in mod:
            return "SPA Routing"
        return "Error Page"
...
cover_data = [
    ...
    ("Version:", f"v{VER} — Merged 10 Sheets"),
    ("Date:", today),
    ("Structure:", f"{len(features)} Feature Sheets (Merged)"),
    ("Changes:", "Updated Headline, Login, DS Header. Merged: GIỎ HÀNG→ĐẶT HÀNG, XÁC NHẬN→THANH TOÁN, ERROR→HOME, MY DESIGNS→DESIGN STUDIO. New: Profile, AI Try-on"),
]
