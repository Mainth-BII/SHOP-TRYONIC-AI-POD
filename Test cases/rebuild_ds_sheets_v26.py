"""
Rebuild v26 Excel: Split 'DESIGN STUDIO' sheet into 6 sub-feature sheets.
Reads from restructured test_cases_suite.md, replaces old DS sheet.
Also preserves HOME PAGE sheet from v26.
"""
import openpyxl
...
for f in sorted(os.listdir(BASE_DIR), reverse=True):
    if f.startswith("TC_POD") and f.endswith(".xlsx") and "v26" in f and not f.startswith("~$"):
        SOURCE = os.path.join(BASE_DIR, f)
        break

OUTPUT = os.path.join(BASE_DIR, f"TC_POD-TShirt-Platform_ExecutionSummary_v26_{today}_v4.xlsx")
VER = 26
...
COL_WIDTHS = {
    1:15, 2:12, 3:20, 4:14, 5:40, 6:12, 7:10,
        ...
        if in_table and line.startswith("|") and "TC_" in line:
            ...
            if len(cols) >= 8:
                ...
                results.append({
                    ...
                    "tc_id": tc_id, "us_map": us_map,
                    "feature": feature_name, "module": module,
                    "title": title, "type": tc_type, "priority": priority,
...
def create_sheet(wb, sheet_name, feature_name, rows):
    ...
    for ci, h in enumerate(HEADERS, 1):
        ...
        if "Review_Manual" in h:
            cell.font = frv; cell.fill = hf_review
        else:
    ...
    ws.freeze_panes = "A3"
    print(f"   ✅ {sheet_name}: {len(rows)} TCs | {len(cat_set)} categories")
    return len(rows)


# DS sub-features mapping: (MD feature name, Excel sheet name)
DS_FEATURES = [
    ...
    ("DESIGN STUDIO - Tạo ảnh AI", "DS - Tạo ảnh AI"),
    ("DESIGN STUDIO - Đặt hàng", "DS - Đặt hàng"),
]


def main():
    if not SOURCE:
        print("❌ No v26 source Excel found!"); return
    ...
    # Delete old DESIGN STUDIO sheet
    if "DESIGN STUDIO" in wb.sheetnames:
        del wb["DESIGN STUDIO"]
        print("   🗑️  Deleted old 'DESIGN STUDIO' sheet")

    # Rebuild HOME PAGE
    home_start = content.find("## 🚀 Feature: HOME PAGE")
    ds_first = content.find("## 🚀 Feature: DESIGN STUDIO")
    if home_start >= 0 and ds_first >= 0:
        home_lines = content[home_start:ds_first].split('\n')
        print("\n🔍 Parsing HOME PAGE...")
    ...
    # Parse and create each DS sub-feature sheet
    total_ds = 0
    print("\n🔍 Parsing DESIGN STUDIO sub-features...")
    for feat_name, sheet_name in DS_FEATURES:
        marker = f"## 🚀 Feature: {feat_name}"
        start = content.find(marker)
        if start < 0:
            print(f"   ⚠️  Feature '{feat_name}' not found in MD")
            continue

        # Find end: next ## 🚀 Feature or EOF
        next_feat = content.find("## 🚀 Feature:", start + len(marker))
        if next_feat < 0:
            section = content[start:]
        else:
            section = content[start:next_feat]

        lines = section.split('\n')
        rows = parse_md_feature(lines, feat_name)
        print(f"   📦 {feat_name}: {len(rows)} TCs")

        if rows:
            count = create_sheet(wb, sheet_name, feat_name, rows)
            total_ds += count

    # Move all DS sheets to end
    for _, sn in DS_FEATURES:
    ...
    if "Change History" in wb.sheetnames:
        ...
        hist_data = [f"v{VER}.0", today,
            f"Restructured DESIGN STUDIO into 6 sub-feature sheets: Chung, Sản phẩm, Ảnh của bạn, Thư viện, Tạo ảnh AI, Đặt hàng. Total DS TCs: {total_ds}.",
            "QA Team"]
    ...
    wb.save(OUTPUT)
    print(f"\n🎉 v{VER} rebuilt: {os.path.basename(OUTPUT)}")
    print(f"   DS sub-features: {total_ds} TCs across {len(DS_FEATURES)} sheets")
