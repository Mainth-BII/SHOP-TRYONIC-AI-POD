"""
ET002 Áo Phông Trẻ Em / Màu Trắng — Luồng In / Mua ngay (MH1→MH10)

Luồng: Listing → Detail → Studio (thiết kế mặt trước) → Review → Popup → Mua ngay → Checkout
       → QR → Order → Admin

Giá ET002 / Trắng / Size 120 / qty 1 (variant 100-140):
  salePrice_áo  = 96.000đ
...
_SALE_AO      = _VARIANT["salePrice"]                           # 96_000
...
class TestDesignBuynowET002Trang(BasePriceFlowTest):
    """ET002 Trắng — luồng In / Mua ngay MH1→MH10."""
    ...
    _REPORT_TITLE = "ET002 Áo Phông Trẻ Em (Trắng) — In / Mua ngay"
    ...
    def setup(self, home_page, product_list_page, product_detail_page,
        ...
        self.tc       = "ET002_TRANG_DESIGN_BUYNOW"
        self.root     = "production"
        self.domain   = "et002_trang_design_buynow"
        self._results = []
    ...
    def _read_review_prices(self) -> dict:
        ...
        return self.page.evaluate(r"""() => {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (/áo phông|trẻ em|giá áo/i.test(line) && !ao_total) {
                    let m = line.match(priceRe);
                    if (!m && i+1 < lines.length) m = lines[i+1].match(priceRe);
            ...
            if (ao_total === 0 && allPrices.length > 0) {
                const v = allPrices.find(p => p >= 80000 && p < sum_total);
                ao_total = v || 96000;
            }
    ...
    def test_design_buynow(self):
        """ET002 Trắng / In / Mua ngay — MH1→MH10."""
        tc = self.tc
        self._login()
