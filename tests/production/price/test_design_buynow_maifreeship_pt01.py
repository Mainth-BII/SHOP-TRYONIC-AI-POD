"""
PT01 Áo Phông Cá Tính / Màu Trắng — Luồng In / Mua ngay / MAIFREESHIP (MH1→MH10)
...
Giá PT01 / Trắng / Size M / qty 1:
  ...
  MAIFREESHIP: miễn phí giao hàng (ship = 0đ)
"""
import json
import os
import re
...
class TestDesignBuynowMaifreeshipPT01(BasePriceFlowTest):
    """PT01 Trắng — luồng In / Mua ngay / MAIFREESHIP MH1→MH10."""
    ...
    _REPORT_TITLE = "PT01 Áo Phông Cá Tính (Trắng) — In / Mua ngay / MAIFREESHIP"
    ...
    def setup(self, home_page, product_list_page, product_detail_page,
        ...
        self.tc       = "PT01_TRANG_DESIGN_BUYNOW_MAIFREESHIP"
        self.root     = "production"
        self.domain   = "pt01_trang_design_buynow_maifreeship"
        self._results = []
    ...
    def test_design_buynow(self):
        """PT01 Trắng / In / Mua ngay / MAIFREESHIP — MH1→MH10."""
        tc = self.tc
        self._login()
        ...
        exp_shipping_after = 0  # MAIFREESHIP → miễn ship

        if shipping_after is not None and shipping_after == 0:
            dc_ok = True
            self._assert_price(shipping_after, 0, "MH5 Phí giao hàng sau MAIFREESHIP (=0đ)")
            exp_total_free = unit_sale_price + int(unit_sale_price * _VAT_RATE) + 0
            total_free = self.checkout.read_checkout_total()
            self._assert_price(total_free, exp_total_free, "MH5 Tổng TT sau MAIFREESHIP")
            exp_discount = _SHIPPING  # 20_000
            actual_total_paid = self.checkout.read_payment_button_price() or exp_total_free
            print(f"  [PASS] MH5: MAIFREESHIP OK — ship=0đ, total={actual_total_paid:,}đ")
        else:
            print("  [INFO] MH5: MAIFREESHIP không áp dụng — tiếp tục với phí ship gốc")
            dc_ok = False
            exp_discount = None
        ...
        # MH7 / MH8 / MH9 / MH10
        # ════════════════════════════════════════════════════════════════════
        self._do_mh7_order(actual_total_paid, 0 if dc_ok else _SHIPPING)
        # MH8: My Orders hiển thị subtotal (áo+in), không phải total paid
        self._do_mh8_my_orders(unit_sale_price)
        self._do_mh9_order_detail(
            order_info, actual_total_paid, 0 if dc_ok else _SHIPPING,
            dc_ok, exp_discount,
        )
        self._do_admin_verify("MH10", order_code, order_info, actual_total_paid, 0 if dc_ok else _SHIPPING)
