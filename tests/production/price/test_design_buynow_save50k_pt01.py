"""
PT01 Áo Phông Cá Tính / Màu Trắng — Luồng In / Mua ngay / SAVE50K (MH1→MH10)
...
Giá PT01 / Trắng / Size M / qty 1:
  ...
  SAVE50K: giảm cố định min(50.000đ, subtotal)
"""
import json
import os
import re
...
class TestDesignBuynowSave50kPT01(BasePriceFlowTest):
    """PT01 Trắng — luồng In / Mua ngay / SAVE50K MH1→MH10."""
    ...
    _REPORT_TITLE = "PT01 Áo Phông Cá Tính (Trắng) — In / Mua ngay / SAVE50K"
    ...
    def setup(self, home_page, product_list_page, product_detail_page,
        ...
        self.tc       = "PT01_TRANG_DESIGN_BUYNOW_SAVE50K"
        self.root     = "production"
        self.domain   = "pt01_trang_design_buynow_save50k"
        self._results = []
    ...
    def test_design_buynow(self):
        """PT01 Trắng / In / Mua ngay / SAVE50K — MH1→MH10."""
        tc = self.tc
        self._login()
        ...
        dc_ok = False
        # Không apply GIAM20
        # Apply SAVE50K thay thế
        self.checkout.apply_discount_code("SAVE50K")
        self.page.wait_for_timeout(2000)
        self._shot("MH5_2", "checkout_after_SAVE50K")

        discount_amt = self.checkout.read_checkout_discount()
        exp_discount = min(50_000, unit_sale_price)

        if discount_amt and discount_amt > 0:
            dc_ok = True
            self._assert_price(discount_amt, exp_discount, "MH5 Giảm giá SAVE50K (50.000đ)")
            after_dc  = unit_sale_price - exp_discount
            vat_dc    = int(after_dc * _VAT_RATE)
            exp_total_dc = after_dc + vat_dc + _SHIPPING
            total_dc = self.checkout.read_checkout_total()
            self._assert_price(total_dc, exp_total_dc, "MH5 Tổng TT sau SAVE50K")
            print(f"  [PASS] MH5: SAVE50K OK — giảm {discount_amt:,}đ")
        else:
            print("  [INFO] MH5: Mã SAVE50K không áp dụng được")
            exp_discount = 0
            exp_total_dc = exp_total_nd  # fallback giá gốc
